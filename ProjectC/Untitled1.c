...
void searchPatient(Patient *patients,int length){
	...
	for(i = 0;i<strlen(searchTemp);i++){
		if(searchTemp[i] >= 'A' && searchTemp[i]<= 'Z'){
			searchTemp[i] += 32; // Ktra chu thuong
		}
	}
	// Chuyen ve chu thuong
	int isHeader = 0;
...
void medicalCheck(Patient *patients,int length,Record *records,int *recordCount){
    ...
    do{
    	...
    	if(strlen(inputDate) == 0){
			...
			int isCheck = 0; // bien check da kham ngay nay chua
			for(i = 0;i<*recordCount;i++){
			...
			if(isCheck){
				printf("Loi: Benh nhan %s da co lich kham vao ngay %s roi!\n", id, inputDate);
                return;
			}else{
				validDate = 1;
			}
...
int main(void){
	...
	do{
		...
		switch(choice){
			...
			case 2:{
				updatePatientInfor(patients, length, records, recordCount); break;
			}
			case 3:{
